...
# Устанавливаем системные утилиты (на случай если локальные подведут)
echo "Установка системных библиотек..."
sudo apt-get update > /dev/null
sudo apt-get install -y android-sdk-libsparse-utils e2fsprogs > /dev/null
...
# Функция запаковки через make_ext4fs
make_ext4() {
    DIR="$1"
    NAME="$2"
    ...
    if [ -d "$DIR" ]; then
        ...
        echo "Запаковка $NAME.img (Размер: ${NEW_SIZE_MB}M)..."
        
        # 1. Сначала пробуем скачанный инструмент
        if [ -x "$TOOLS_DIR/make_ext4fs" ]; then
            "$TOOLS_DIR/make_ext4fs" -s -L "$NAME" -l "${NEW_SIZE_MB}M" -a "$NAME" "$OUT_DIR/$NAME.img" "$DIR"
            RET=$?
        else
            RET=1
        fi
        
        # 2. Если не вышло (или инструмента нет), пробуем системный
        if [ $RET -ne 0 ]; then
            echo "Локальный инструмент не сработал, пробуем системный..."
            if command -v make_ext4fs &> /dev/null; then
                 make_ext4fs -s -L "$NAME" -l "${NEW_SIZE_MB}M" -a "$NAME" "$OUT_DIR/$NAME.img" "$DIR"
            else
                 echo "ОШИБКА: make_ext4fs не найден нигде!"
            fi
        fi
    fi
}
